...
void ChatServiceCodeC::encode(corpc::TcpBuffer *buf, corpc::AbstractData *data)
{
    ...
    USER_LOG_DEBUG << "test custom encode end, pkType = " << (int)pkTypeChar << ", pkLen = " << dataSize;
}

void ChatServiceCodeC::decode(corpc::TcpBuffer *buf, corpc::AbstractData *data)
{
    USER_LOG_DEBUG << "test custom decode start";
    if (!buf || !data) {
        USER_LOG_ERROR << "decode error! buf or data nullptr";
        return;
    }
    ChatServiceStruct *chatServiceStruct = dynamic_cast<ChatServiceStruct *>(data);
    if (!chatServiceStruct) {
        USER_LOG_ERROR << "not ChatServiceStruct type";
        return;
    }
    ...
    USER_LOG_DEBUG << "test custom decode end, pkType = " << (int)pkType << ", pkLen = " << pkLen;
}
